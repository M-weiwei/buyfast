...
(function ($, window) {
  let ele = null
  let exzoom_img_box = null
  let boxWidth = null
  let boxHeight = null
  let exzoom_img_ul_outer = null;
  let // 用于限制 ul 宽度,又不影响放大镜区域
    exzoom_img_ul = null
  let exzoom_img_ul_position = 0;
  let // 循环图片区域的边距,用于移动时跟随光标
    exzoom_img_ul_width = 0;
  let // 循环图片区域的最大宽度
    exzoom_img_ul_max_margin = 0;
  let // 循环图片区域的最大外边距,应该是图片数量减一乘以boxWidth
    exzoom_nav = null
  let exzoom_nav_inner = null
  const navHightClass = 'current';
  let // 当前图片的类,
    exzoom_navSpan = null
  let navHeightWithBorder = null
  let images = null
  let exzoom_prev_btn = null;
  let // 导航上一张图片
  ...
  let // 图片属性的数字
    exzoom_zoom = null
  let exzoom_main_img = null
  let exzoom_zoom_outer = null
  let exzoom_preview = null;
  let // 预览区域
  ...
  const methods = {
    ...
    setImg: function () { // 设置大图
      ...
      getImageSize(url, function (width, height) {
        ...
        const image_prop = copute_image_prop(url, width, height)
  ...
  function init() {
    ...
    for (let i = 0; i < imgNum; i++) {
      ...
      const li = exzoom_img_ul.find('li').eq(i)
...
<p class='exzoom_preview'>
  ...
  function checkLoadedAllImages(images) {
    const timer = setInterval(function () {
      ...
      const all_images_num = images.length
  ...
  function bindingEvent() {
    ...
    exzoom_img_ul.on('touchstart', function (event) {
      const coords = getCursorCoords(event)
      startX = coords.x
      startY = coords.y

      const left = exzoom_img_ul.css('left')
    ...
    exzoom_zoom_outer.on('mousemove', function (e) {
      const width_limit = exzoom_zoom.width() / 2
      const max_X = exzoom_zoom_outer.width() - width_limit
      const max_Y = exzoom_zoom_outer.height() - width_limit
      const current_X = e.pageX - exzoom_zoom_outer.offset().left
      const current_Y = e.pageY - exzoom_zoom_outer.offset().top
      let move_X = current_X - width_limit
      let move_Y = current_Y - width_limit
  ...
  function move(direction) {
    ...
    // 判断缩略图导航是否需要重新设置偏移量
    const exzoom_nav_width = exzoom_nav.width()
    const nav_item_width = g.navItemMargin + g.navWidth + g.navBorder * 2 // 单个导航元素的宽度
    let new_nav_offset = 0

    // 直接对比当前索引的图片占据的宽度和exzoom的宽度的差作为偏移量即可
    const temp = nav_item_width * (imgIndex + 1)
  ...
  function copute_image_prop(image, width, height) {
    ...
    if (img_scale === 1) {
      ...
      res[9] = boxHeight * 2 // height
      const top = (g.navHeight - (g.navWidth / img_scale)) / 2
      ...
      const top = (g.navWidth - (g.navHeight * img_scale)) / 2
